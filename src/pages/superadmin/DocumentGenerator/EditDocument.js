...
import { editProcedureDocument } from "../../../redux/superAdminReducer/superAdminAction";
...
const EditDocument = () => {
  ...
  const [title, setTitle] = useState(selectedDocument.form.formData.title);
  const [myform, setmyform] = useState(selectedDocument.form.formData);
  ...
  const onSubmitHandler = async () => {
    ...
    const success = await dispatch(
      editProcedureDocument(formData, procedureId)
    );
    if (success) {
      navigate(-1);
    }
