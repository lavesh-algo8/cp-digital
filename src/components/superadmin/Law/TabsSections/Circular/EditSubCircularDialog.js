...
const EditSubCircularDialog = (props) => {
  ...
  const copyData = props?.subcircularDetails;
  ...
  const [subcircularNo, setsubcircularNo] = useState("");
  ...
  const handleSubmit = async (e) => {
    ...
    const data = {
      no: parseInt(subcircularNo),
      sub_circular_heading: subcircularName,
    ...
    console.log(data);
    await dispatch(editSubCircular(data, props?.subcircularDetails._id));
    setsubcircularNo("");
  ...
  useEffect(() => {
    if (props) {
      setsubcircularName(props.subcircularDetails.sub_circular_heading);
      setsubcircularNo(props.subcircularDetails.sub_circular_no);
      setcircularShortDescription(props.subcircularDetails.short_desc);
      setdateOfCircular(props.subcircularDetails.upload_date);
      setsubsectionName([
        props?.subcircularDetails?.sub_section?.sub_section_name,
      ]);
      if (props.subcircularDetails?.section?.section_name != null) {
        setsectionName([props?.subcircularDetails?.section?.section_name]);
        handleSectionSelectionChange(
          12,
          props?.subcircularDetails?.section?._id
        );
      }
      if (props.subcircularDetails?.chapter?.chapter != null) {
        setchapterName([props?.subcircularDetails?.chapter?.chapter]);
        handleChapterSelectionChange(
          23,
          props?.subcircularDetails?.chapter?._id
        );
      }
      setdateOfAmendment(props.subcircularDetails.amendment_date);
      if (props?.subcircularDetails) {
        setValue(
          htmlToDraftBlocks(props.subcircularDetails.sub_circular_details)
        );
      }
    }
  }, [copyData]);
  ...
  return (
    <>
      ...
      <Dialog
        ...
        maxWidth="lg"
      >
        <DialogTitle fontWeight={600}>Edit Sub-Circular </DialogTitle>
          ...
          <form onSubmit={handleSubmit}>
            <Grid container spacing={2}>
              <Grid item lg={5} md={12}>
                ...
                >
                  <Typography>Sub Circular Number</Typography>
                  <OutlinedInput
                    id="outlined-adornment-weight"
                    value={subcircularNo}
                    onChange={(e) => setsubcircularNo(e.target.value)}
