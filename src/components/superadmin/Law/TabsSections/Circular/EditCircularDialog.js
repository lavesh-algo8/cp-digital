...
const EditCircularDialog = (props) => {
  const circularId = props.circularsDetails._id;
  ...
  useEffect(() => {
    setcircularName(props?.circularsDetails.circular_heading);
    setcircularNo(props?.circularsDetails.circular_no);
    setcircularShortDescription(props?.circularsDetails.short_desc);
    setdateOfCircular(props?.circularsDetails.circular_date);
    setsubsectionName([]);
    if (props?.circularsDetails.sub_sections) {
      setsubsectionName(props?.circularsDetails?.sub_sections.split(","));
    }
    if (props?.circularsDetails) {
      setValue(htmlToDraftBlocks(props?.circularsDetails?.circular_desc));
    }
  }, [props]);
