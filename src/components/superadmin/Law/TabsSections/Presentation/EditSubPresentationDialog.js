...
const EditSubPresentationDialog = (props) => {
  const copyData = props?.subpresentationDetails;
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(data);
    await dispatch(
      EditSubPresentation(data, props?.subpresentationDetails._id)
    );
    setsubpresentation("");
  ...
  useEffect(() => {
    if (props) {
      setsubpresentation(props.subpresentationDetails.sub_title);
      setpresentationAuthor(props.subpresentationDetails.author);
      setdateOfPresentation(props.subpresentationDetails.date);
      setsubsectionName([
        props?.subpresentationDetails?.sub_section?.sub_section_name,
      ]);
      if (props.subpresentationDetails?.section?.section_name != null) {
        setsectionName([props?.subpresentationDetails?.section?.section_name]);
        handleSectionSelectionChange(
          12,
          props?.subpresentationDetails?.section?._id
        );
      }
      if (props.subpresentationDetails?.chapter?.chapter != null) {
        setchapterName([props?.subpresentationDetails?.chapter?.chapter]);
        handleChapterSelectionChange(
          23,
          props?.subpresentationDetails?.chapter?._id
        );
      }
    }
  }, [copyData]);
  ...
  return (
    <>
      ...
      <Dialog
        ...
        maxWidth="lg"
      >
        <DialogTitle fontWeight={600}>Edit Sub-Presentation </DialogTitle>
...
export default EditSubPresentationDialog;
