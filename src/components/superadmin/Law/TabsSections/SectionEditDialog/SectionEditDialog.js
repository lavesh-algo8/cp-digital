...
const SectionEditDialog = (props) => {
  ...
  const [regulationName, setregulationName] = useState(
    props.sectionDetails?.regulation
  );
  const [regulationNo, setregulationNo] = useState(
    props.sectionDetails?.regulation_no
  );
  const [dateOfUpdate, setdateOfUpdate] = useState(
    props.sectionDetails?.updatedAt
  );
  ...
  const [updatedBy, setupdatedBy] = useState(props.sectionDetails?.updatedBy);
  const [sorting, setsorting] = useState(props.sectionDetails?.sorting);
  const [value, setValue] = useState(
    htmlToDraftBlocks(props?.sectionDetails?.regulation_details)
  );
  ...
  useEffect(() => {
    ...
    setregulationName(props.sectionDetails.regulation);
    setregulationNo(props.sectionDetails.regulation_no);
    setdateOfUpdate(props.sectionDetails.updatedAt);
    setupdatedBy(props.sectionDetails.updatedBy);
    setsorting(props.sectionDetails.sorting);
    if (props?.sectionDetails) {
      setValue(htmlToDraftBlocks(props?.sectionDetails?.regulation_details));
    }
  }, [props]);
          ...
          <form onSubmit={handleSubmit}>
            <Grid container spacing={2}>
              <Grid item lg={5} md={12}>
                ...
                >
                  <Typography sx={{ mt: 2 }}>Name of the section</Typography>
                  <OutlinedInput
