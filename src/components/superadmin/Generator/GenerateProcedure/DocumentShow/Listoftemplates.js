...
import Tempheadingspopup from "./Tempheadingspopup";
...
const Listoftemplates = () => {
  ...
  const { listOfTemplates, templateDocOnly } = useSelector(
  ...
  const [openHeadingsDialog, setOpenHeadingsDialog] = useState(false);

  const handleCheckBox = async (row) => {
    console.log(row);
    console.log({
      templateId: row._id,
      title: row.templateformdata.title,
    });
    const finalData = {
      templateId: row._id,
      title: row.templateformdata.title,
    ...
    if (templateDocOnly?.some((item) => item.templateId === row._id)) {
      await dispatch({
          ...
          <>
            {params?.row?.templateformdata ? (
              <>
                ...
                <Checkbox
                  checked={templateDocOnly?.some(
                    (item) => item.templateId === params?.row._id
                  )}
                  onClick={(e) => handleCheckBox(params?.row)}
                  color="secondary"
                />
              </>
            ) : (
  ...
  return (
    <>
      ...
      {openHeadingsDialog && (
        <Tempheadingspopup
          openDialog={openHeadingsDialog}
          setOpenDialog={setOpenHeadingsDialog}
        />
      )}
